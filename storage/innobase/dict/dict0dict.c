...
)
{
    ...
    table->big_rows = dict_table_get_row_len(table) >= BIG_ROW_SIZE;
}
...
*/
ulint
dict_index_max_size_for_tree(
/*========================*/
	const dict_table_t*	table,		/*!< in: table */
	const dict_index_t*	new_index,  /*!< in: index */
    ibool               is_leaf,    /*!< in: leaf node or non-leaf node */
    ulint*              page_rec_max_out)	
{
	ulint	zip_size;
	ulint	comp;
	ulint	i;
    ...
    n_fields = is_leaf ? new_index->n_fields : dict_index_get_n_unique_in_tree(new_index);
...
// 		}
    ...
    if (!is_leaf) 
    {
        rec_max_size += REC_NODE_PTR_SIZE;
        *page_rec_max_out = page_ptr_max;
    }
    else
        *page_rec_max_out = page_rec_max;
